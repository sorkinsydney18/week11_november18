...
analyze pid.strength - hypothesis: stronger the partisan the more affected by the partisan valence, less likely to seek outside source to inform correct answer, more entrenched in their opinion 

education level could be affected by partisan level though


```{r}
data %>% 
  filter(female == 1) %>% 
  ggplot(aes(correct.answer)) +
  geom_histogram(stat = "count") +
  facet_wrap(~incentive) +
  scale_x_continuous(breaks = c(0, .5, 1), labels = c("Incorrect", "", "Correct")) +
  labs(title = "Treatment Affect of Female Respondents")
...
data %>% 
  ...
  facet_wrap(~incentive) +
  scale_x_continuous(breaks = c(0, .5, 1), labels = c("Incorrect", "", "Correct")) +
  labs(title = "Treatment Affect of Male Respondents")
